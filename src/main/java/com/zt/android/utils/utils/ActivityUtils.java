...
public final class ActivityUtils {
    ...
    public static boolean isActivityExists(@NonNull final String packageName,
        ...
        return !(Utils.getAppContext().getPackageManager().resolveActivity(intent, 0) == null ||
                intent.resolveActivity(Utils.getAppContext().getPackageManager()) == null ||
                Utils.getAppContext().getPackageManager().queryIntentActivities(intent, 0).size() == 0);
    }
    ...
    public static String getLauncherActivity() {
        return getLauncherActivity(Utils.getAppContext().getPackageName());
    }
    ...
    /**
     * 判断当前应用页面Activity是否在前台
     *
     * @param activity activity
     * @return {@code true}: 是<br>{@code false}: 不是
     */
    public static boolean isActivityInForeground(@NonNull Activity activity) {
        boolean is;
        Activity topActivity = getTopActivity();
        if(topActivity!=null){
            is=topActivity.getClass().getName().equals(activity.getClass().getName())&!Utils.isAppInBackground();
        }else{
            is=false;
        }
        return is;
     ...
     * @return {@code true}: 是<br>{@code false}: 不是
     */
    public static boolean isActivityInForeground(Class<? extends Activity> activity) {
        boolean is;
        Activity topActivity = getTopActivity();
        if(topActivity!=null){
            is=topActivity.getClass().getName().equals(activity.getName())&!Utils.isAppInBackground();
        }else{
            is=false;
        }
        return is;
    ...
    private static Context getActivityOrApp() {
        Activity topActivity = getTopActivity();
        return topActivity == null ? Utils.getAppContext() : topActivity;
    }
